...
public class EHRService {
     ...
     * AI call is outside transaction to prevent connection pooling issues/deadlocks.
     */
    ...
    @Transactional
    public ClinicalNote createInitialNote(User patient, User doctor, ClinicalNote.NoteType type, String text, String fileType, String filePath) {
    ...
    /**
     * Extract medical entities from file.
     */
    public EHRDto.ExtractionResultDto extractFromFile(MultipartFile file, Long patientId, Long doctorId, String noteType) {
        ...
        // Phase 1: Create note
        ClinicalNote note = createInitialNote(patient, doctor, parseNoteType(noteType), null, getFileExtension(file.getOriginalFilename()), null);

        try {
            // Phase 2: Call AI service with file (OUTSIDE transaction)
            Map<String, Object> aiResponse = callAiExtractFile(file);
            
            // Extract raw text from AI response if available
    ...
    @Transactional
    public void handleExtractionFailure(Long noteId, Exception e) {
        clinicalNoteRepository.findById(noteId).ifPresent(note -> {
    ...
    public EHRDto.PatientEHRSummaryDto getPatientEHRSummary(Long patientId) {
        ...
        List<EHRDto.PatientMedicationDto> activeMeds = patientMedicationRepository.findByPatientIdAndStatus(patientId, PatientMedication.MedicationStatus.ACTIVE)
                .stream().map(this::toMedicationDto).collect(Collectors.toList());
        
        List<EHRDto.PatientConditionDto> activeConds = patientConditionRepository.findByPatientIdAndStatus(patientId, PatientCondition.ConditionStatus.ACTIVE)
                .stream().map(this::toConditionDto).collect(Collectors.toList());
        
        List<EHRDto.PatientSymptomDto> recentSymptoms = patientSymptomRepository.findByPatientIdOrderByCreatedAtDesc(patientId)
    ...
    private Map<String, Object> callAiExtractText(String text) {
        Object response = aiServiceWebClient
                ...
                .block(Duration.ofSeconds(30)); // Timeout check
