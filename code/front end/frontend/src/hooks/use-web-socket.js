...
const useWebSocket = (config = {}) => {
  ...
  const doConnect = useCallback(() => {
    ...
    client.onStompError = (frame) => {
      ...
      // Check for permanent authentication errors to prevent hammer reconnection loops
      const isAuthError = errMsg.includes('Unauthorized') || errMsg.includes('Access Denied') || errMsg.includes('Token');
      if (isAuthError) {
        console.warn('[WS] Permanent authentication error detected. Stopping reconnection loops.');
        if (reconnectTimerRef.current) {
          clearTimeout(reconnectTimerRef.current);
          reconnectTimerRef.current = null;
        }
  ...
  const subscribe = useCallback((destination, callback) => {
    ...
    return client.subscribe(destination, (message) => {
      try {
        ...
        console.log('[WS] ◄ RECEIVED on', destination, ':', parsed.senderType || parsed.type);
