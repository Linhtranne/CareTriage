  ...
  return (
    <Box sx={{
      ...
      <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 2 }}>
        ...
        {/* Hero Info */}
        <Box sx={{ mb: 8 }}>
          <Typography variant="overline" sx={{ color: 'primary.dark', fontWeight: 800, letterSpacing: 2 }}>
            {displayCategory}
          </Typography>
          <Typography variant="h2" sx={{ fontWeight: 900, color: 'primary.900', mt: 1, mb: 3 }}>
            {displayTitle}
          </Typography>
          <Typography variant="h6" sx={{ color: 'text.secondary', maxWidth: 800, lineHeight: 1.8, fontWeight: 500 }}>
            {displayDesc}
          </Typography>
        </Box>
          ...
          {displayFeatures.map((feat, index) => (
            <Grid size={{ xs: 12, md: 4 }} key={index}>
              <Paper sx={{
                ...
                '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 30px 60px color-mix(in srgb, var(--color-primary-500) 10%, transparent)' }
              }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, color: 'primary.dark' }}>
                  <CheckCircleOutlined sx={{ fontSize: 28, mr: 1.5 }} />
                  <Typography variant="h6" sx={{ fontWeight: 800, color: 'primary.900' }}>
                    {feat.title}
                  </Typography>
                </Box>
        ...
        <Paper sx={{
          p: 6, borderRadius: 2,
          background: gradient,
          color: 'background.paper', textAlign: 'center',
          boxShadow: '0 20px 40px color-mix(in srgb, var(--color-primary-500) 30%, transparent)'
        }}>
          ...
          <Button
            ...
            sx={{
              backgroundColor: 'background.paper', color: 'primary.900',
              fontWeight: 800, px: 5, py: 1.5, borderRadius: 3,
              '&:hover': { backgroundColor: 'background.default', transform: 'scale(1.05)' },
              transition: 'all 0.2s'
            }}
          >
            Bắt đầu miễn phí
