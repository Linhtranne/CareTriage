...
import {
  ...
  Divider, List, ListItemText, ListItemAvatar,
...
const getIcon = (type) => {
  switch (type) {
    case 'CHAT': return <ChatIcon sx={{ color: 'primary.main' }} />;
    case 'APPOINTMENT': return <AppointmentIcon sx={{ color: 'var(--color-info)' }} />;
    case 'MEDICAL_RECORD': return <RecordIcon sx={{ color: '#08bba3' }} />;
    case 'TRIAGE': return <TriageIcon sx={{ color: 'var(--color-warning)' }} />;
...
export default function NotificationBell() {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'color-mix(in srgb, var(--color-primary-500) 4%, transparent)' }}>
          <Typography variant="subtitle1" sx={{ fontWeight: 800, color: 'primary.800' }}>
          ...
          {unreadCount > 0 && (
            <Button 
              ...
              sx={{ 
                ...
                fontWeight: 700,
                color: 'primary.main',
                '&:hover': { bgcolor: 'color-mix(in srgb, var(--color-primary-500) 8%, transparent)' }
              }}
            >
              ...
              {notifications.map((n) => (
                <MenuItem
                  ...
                  sx={{
                    py: 1.5,
                    px: 2,
                    borderLeft: n.isRead ? '4px solid transparent' : '4px solid var(--color-primary-500)',
                    bgcolor: n.isRead ? 'transparent' : 'color-mix(in srgb, var(--color-primary-500) 2%, transparent)',
                    transition: 'all 0.2s',
                ...
                >
                  <ListItemAvatar sx={{ minWidth: 48, mt: 0.5 }}>
                    <Avatar sx={{ bgcolor: 'color-mix(in srgb, var(--color-primary-500) 10%, transparent)', width: 36, height: 36 }}>
                      {getIcon(n.type)}
                    </Avatar>
                  </ListItemAvatar>
                  <ListItemText
