...
export default function BookAppointment() {
  ...
  async function fetchDoctors(deptId?: number) {
    try {
      const res = await publicApi.getDoctors({ departmentId: deptId, size: 100 });
      const internalDocs = res.data.data?.content || [];
      
      const extRes = await publicApi.getExternalDoctors();
      ...
      if (deptId) {
        ...
        if (dept) {
          externalDocs = externalDocs.filter((d: any) => 
            d.specialization?.toLowerCase().includes(dept.name.toLowerCase())
          );
        }
      }
      
      const taggedExt = externalDocs.map((d: any) => ({ ...d, isExternal: true }));
      const allDocs = [...internalDocs, ...taggedExt];
      setDoctors(allDocs as any);
    } catch (err) {
  ...
  const handleBooking = async () => {
    ...
    try {
      const bookingData = {
        doctorId: selectedDoctor.isExternal ? null : selectedDoctor.id,
        externalDoctorId: selectedDoctor.isExternal ? selectedDoctor.id : null,
                ...
                <Grid size={12}>
                  <Box sx={{ p: 3, bgcolor: 'white', borderRadius: 4, border: '1px solid oklch(92% 0.02 250)' }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 800, mt: 1, color: 'oklch(20% 0.05 250)' }}>
                      {user?.fullName} <span style={{ opacity: 0.5, margin: '0 8px' }}>ΓÇó</span> {user?.phone}
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
