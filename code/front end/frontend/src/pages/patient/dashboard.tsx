...
import { CalendarDays, ChevronRight, ClipboardList, FileText, PhoneCall, Clock } from 'lucide-react'
                    ...
                    <Grid container spacing={3}>
                        <Grid size={{ xs: 12, sm: 6, md: 6 }}>
                            <DashboardCard
                                ...
                                value={upcomingAppointment?.appointmentTime || t('common.not_provided')}
                                subtitle={upcomingAppointment?.doctorName || t('dashboard.no_upcoming')}
                                icon={Clock}
                                color="#2563eb"
                        ...
                        <Grid size={{ xs: 12, lg: 5 }}>
                            <Box sx={{
                                ...
                                <Stack spacing={0}>
                                    {(() => {
                                        if (loading) {
                                            return [1, 2, 3, 4].map(i => <Box key={i} sx={{ height: 80, mb: 2, bgcolor: 'oklch(96% 0.01 250)', borderRadius: 3, animation: 'pulse 1.5s infinite' }} />);
                                        }
                                        if (records.length === 0) {
                                            return (
                                        ...
                                        return records.slice(0, 5).map((record, idx) => (
                                            <Box
                                                key={record.id}
