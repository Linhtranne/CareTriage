...
import {
  ...
  MenuItem, Alert, IconButton, InputAdornment, Tooltip, Avatar,
  Zoom
} from '@mui/material'
import {
...
export default function TriageTicketInbox() {
  ...
  const [confirmedUrgency, setConfirmedUrgency] = useState('MEDIUM')
  ...
  const handleOpenDetail = async (ticket) => {
    ...
    try {
      ...
      const detailedTicket = detailRes.data?.data
      ...
      setConfirmedSummary(detailedTicket.doctorConfirmedSummary || detailedTicket.description || '')
      setConfirmedDeptId(detailedTicket.categoryId || '')
      setConfirmedUrgency(detailedTicket.confirmedUrgency || detailedTicket.priority || 'MEDIUM')
  ...
  const handleDoctorReview = async () => {
    ...
    try {
      const res = await triageTicketApi.doctorReview(selectedTicket.id, {
        reviewStatus,
        confirmedSummary,
      ...
      setReviewSuccess('Đã lưu kết quả phê duyệt lâm sàng thành công!')
      setSelectedTicket(res.data?.data)
      setTimeout(() => {
        loadTickets()
      }, 1500)
    } catch (error) {
      setReviewError(error.response?.data?.message || 'Không thể lưu phê duyệt lâm sàng. Vui lòng kiểm tra lại.')
    } finally {
  ...
  const handleCreateAppointment = async () => {
    ...
    try {
      ...
      setCreateError(error.response?.data?.message || 'Không thể tạo lịch hẹn. Vui lòng kiểm tra lại thời gian.')
    } finally {
        ...
        <DialogContent sx={{ p: 0, overflow: 'hidden' }}>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, minHeight: 600 }}>
            ...
            <Box sx={{ 
              ...
              {/* Patient Info Card */}
              <Box sx={{ p: 2.5, bgcolor: 'oklch(100% 0 0 / 0.2)', borderRadius: 5, border: '1px solid oklch(100% 0 0 / 0.1)' }}>
              ...
              {/* Doctor Review Card */}
              <Box sx={{ p: 2.5, bgcolor: 'oklch(100% 0 0 / 0.2)', borderRadius: 5, border: '1px solid oklch(65% 0.15 160 / 0.2)' }}>
                <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 1, color: 'oklch(30% 0.05 160)' }}>
                  <Stethoscope size={18} /> Phê duyệt lâm sàng của Bác sĩ
                </Typography>
                
                {reviewError && <Alert severity="error" sx={{ mb: 2, borderRadius: 3 }}>{reviewError}</Alert>}
                {reviewSuccess && <Alert severity="success" sx={{ mb: 2, borderRadius: 3 }}>{reviewSuccess}</Alert>}

                {selectedTicket?.doctorReviewStatus === 'APPOINTMENT_CREATED' ? (
                  <Alert severity="success" icon={<CheckCircle2 size={20} />} sx={{ borderRadius: 3, mb: 2 }}>
                    Đã chuyển đổi thành lịch hẹn khám bệnh thành công. Chat session đã khóa.
                  </Alert>
                ) : null}
                ...
                <Stack spacing={2}>
                  <TextField
                    ...
                    fullWidth
                    disabled={selectedTicket?.doctorReviewStatus === 'APPOINTMENT_CREATED'}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: 'oklch(100% 0 0 / 0.1)', '& fieldset': { borderColor: 'oklch(100% 0 0 / 0.1)' } } }}
                  >
                    <MenuItem value="AI_ANALYSIS_PENDING_REVIEW">Chờ Bác sĩ phê duyệt</MenuItem>
                    <MenuItem value="DOCTOR_CONFIRMED">Xác nhận kết quả AI</MenuItem>
                    <MenuItem value="DOCTOR_EDITED">Bác sĩ đã điều chỉnh</MenuItem>
                    <MenuItem value="NEEDS_MORE_INFORMATION">Cần thêm thông tin y khoa</MenuItem>
                    <MenuItem value="REJECTED">Từ chối / Hủy bỏ</MenuItem>
                  </TextField>

                  <TextField
                    select
                    label="Chuyên khoa xác nhận"
                    size="small"
                    value={confirmedDeptId}
                    onChange={(e) => setConfirmedDeptId(e.target.value)}
                    fullWidth
                    disabled={selectedTicket?.doctorReviewStatus === 'APPOINTMENT_CREATED'}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: 'oklch(100% 0 0 / 0.1)', '& fieldset': { borderColor: 'oklch(100% 0 0 / 0.1)' } } }}
                  >
                  ...
                  <TextField
                    select
                    label="Mức độ khẩn cấp lâm sàng"
                    size="small"
                    ...
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 4, bgcolor: 'oklch(100% 0 0 / 0.1)', '& fieldset': { borderColor: 'oklch(100% 0 0 / 0.1)' } } }}
                  >
                    <MenuItem value="LOW">Bình thường (Low)</MenuItem>
                    <MenuItem value="MEDIUM">Trung bình (Medium)</MenuItem>
                    <MenuItem value="HIGH">Cao (High)</MenuItem>
                    <MenuItem value="URGENT">Khẩn cấp (Urgent)</MenuItem>
                  </TextField>

                  <TextField
                    label="Tóm tắt bệnh án sơ bộ chính thức"
                    multiline
                    rows={3}
                    size="small"
                  ...
                  <TextField
                    label="Ghi chú lâm sàng của bác sĩ"
                    multiline
                    rows={2}
                    size="small"
                    value={doctorNotes}
                  ...
                  {selectedTicket?.doctorReviewStatus !== 'APPOINTMENT_CREATED' && (
                    ...
                    >
                      {reviewLoading ? 'Đang lưu phê duyệt...' : 'Lưu kết quả phê duyệt lâm sàng'}
                    </Button>
                  )}
                </Stack>
              </Box>
              ...
              {/* Appointment Card */}
              {(selectedTicket?.doctorReviewStatus === 'DOCTOR_CONFIRMED' || 
                selectedTicket?.doctorReviewStatus === 'DOCTOR_EDITED' || 
                selectedTicket?.doctorReviewStatus === 'APPOINTMENT_CREATED') && (
                <Box sx={{ p: 2.5, border: '1px solid oklch(100% 0 0 / 0.1)', borderRadius: 5, bgcolor: 'oklch(100% 0 0 / 0.2)' }}>
                  <Typography variant="subtitle2" sx={{ mb: 2.5, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 1, color: 'oklch(30% 0.05 160)' }}>
                    <Calendar size={18} /> Chuyển đổi thành lịch hẹn khám
                  </Typography>
                  ...
                  {selectedTicket?.doctorReviewStatus === 'APPOINTMENT_CREATED' ? (
                    <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 600, textAlign: 'center', py: 2 }}>
                      Lịch hẹn đã được thiết lập thành công. Vui lòng kiểm tra Lịch hẹn của ngày.
                    </Typography>
                  ) : (
                    <Stack spacing={2.5}>
                      <TextField
                      ...
                      <TextField
                        select
                        label="Chuyên khoa khám thực tế"
                        size="small"
                        value={departmentId}
                      ...
                      >
                        {createLoading ? 'Đang xử lý...' : 'Xác nhận tạo lịch hẹn khám'}
                      </Button>
                    </Stack>
                  )}
                </Box>
