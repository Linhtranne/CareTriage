...
export default function EHRUpload() {
  ...
  const selectedNoteTypeLabel = NOTE_TYPES.find((noteType) => noteType.value === selectedNoteType)?.label;
  ...
  const onFileChange = (e) => {
    ...
    if (!validTypes.includes(selectedFile.type) && !validExtensions.includes(extension)) {
      setFileError('Chỉ hỗ trợ PDF, Word (.doc, .docx) hoặc Văn bản (.txt)');
      setFile(null);
      setTxtContent('');
      return;
    }
    ...
    // Handle plain text (.txt) preview reading
    if (selectedFile.type === 'text/plain' || extension === 'txt') {
      const reader = new FileReader();
      reader.onload = (event) => {
        setTxtContent(event.target.result);
      };
      reader.onerror = () => {
        setFileError('Không thể đọc nội dung file văn bản');
      };
            ...
            <Box sx={{ p: 0 }}>
              <AnimatePresence mode="wait">
                {tabValue === 0 ? (
                  ...
                  <Box key="file-mode" data-testid="file-mode" component={motion.div} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} sx={{ p: 3 }}>
                    <Box sx={{ display: 'flex', gap: 3, alignItems: 'stretch', width: '100%' }}>
                      ...
                      >
                        <input type="file" hidden ref={fileInputRef} onChange={onFileChange} accept=".pdf,.doc,.docx,.txt" />
                        <Box sx={{
                          width: '100%',
                          display: 'flex',
                          flexDirection: 'column',
                          ...
                          {file ? (
                            <>
                              ...
                              <Button
                                ...
                                size="small"
                                startIcon={<X size={14} />}
                                onClick={(e) => { e.stopPropagation(); setFile(null); setTxtContent(''); }}
                                sx={{
                                  fontWeight: 600,
                                  borderRadius: 2,
                                  textTransform: 'none',
                            ...
                            <>
                              <Upload size={56} color={fileError ? 'error' : 'oklch(65% 0.15 160)'} />
                              <Typography variant="subtitle1" sx={{ fontWeight: 600, color: fileError ? 'error.main' : 'oklch(20% 0.05 160)' }}>
                                {fileError ? 'Tập tin không hợp lệ' : 'Tải hồ sơ lên'}
                              </Typography>
                              ...
                              {fileError && (
                                <Typography variant="caption" sx={{ color: 'error.main', fontWeight: 600, px: 2 }}>
                                  {fileError}
                                </Typography>
                              )}
                            </>
                        ...
                        {!file ? (
                          ...
                          <>
                            {file.type === 'application/pdf' ? (
                              ...
                              >
                                <Typography variant="subtitle2" sx={{ fontWeight: 800, color: 'oklch(55% 0.18 160)', mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                                  <FileText size={18} /> Xem trước nội dung văn bản gốc:
                                </Typography>
                                <Divider sx={{ mb: 2 }} />
                                {txtContent || 'Đang tải nội dung...'}
                              </Box>
                            ) : (
                              <Stack
                                sx={{ height: '100%', p: 4 }}
                              ...
                              >
                                ...
                                <Box>
                                  ...
                                  <Typography variant="body1" sx={{ fontWeight: 600, color: 'oklch(50% 0.02 160)' }}>
                                    AI vẫn có thể trích xuất chính xác nội dung từ tệp này
                                  </Typography>
                                </Box>
                                <Typography variant="caption" sx={{ color: 'oklch(60% 0.02 160)', maxWidth: '80%' }}>
                                  Do giới hạn kỹ thuật của trình duyệt đối với định dạng Microsoft Word. Vui lòng sử dụng PDF hoặc TXT nếu bạn muốn xem trước trực tiếp.
                                </Typography>
                              </Stack>
                            )}
                          </>
                  ...
                  <Grid size={{ xs: 12, md: 5 }} >
                    <Box sx={{
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600, color: 'oklch(30% 0.05 160)', lineHeight: 1.4, fontSize: '0.85rem' }}>
                        Áp dụng AI Engine chuyên biệt cho <strong>{selectedNoteTypeLabel}</strong> để tối ưu độ chính xác.
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
