...
export default function Emergency() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', pb: 10 }}>
      ...
      <Box sx={{ 
        pt: 15, pb: 6, px: 3, bgcolor: '#991b1b', color: 'background.paper', 
      ...
      <Container maxWidth="xl" sx={{ mt: 6 }}>
        <Grid container spacing={4}>
            ...
            <Stack spacing={3}>
              <Paper elevation={0} sx={{ 
                ...
                <Button 
                  ...
                  fullWidth
                  sx={{ 
                    bgcolor: 'var(--color-danger)', color: 'background.paper', py: 3, 
                    borderRadius: 1, fontSize: '3rem', fontWeight: 900,
                ...
                <Grid container spacing={2}>
                  {firstAidSteps.map((step, index) => (
                    <Grid size={{ xs: 12 }} key={index}>
                      <Box sx={{ p: 3, bgcolor: 'background.default', borderRadius: 1, borderLeft: '6px solid #ef4444' }}>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <Stack spacing={3}>
              <Paper elevation={0} sx={{ p: 4, borderRadius: 2, bgcolor: 'text.primary', color: 'background.paper' }}>
                ...
                <Stack spacing={2} sx={{ mb: 4 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, bgcolor: 'color-mix(in srgb, var(--color-surface-50) 5%, transparent)', borderRadius: 3 }}>
                    <Typography>Khoảng cách:</Typography>
                    <Typography sx={{ fontWeight: 700, color: 'primary.main' }}>{distance ? `${distance} km` : 'Đang định vị...'}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', p: 2, bgcolor: 'color-mix(in srgb, var(--color-surface-50) 5%, transparent)', borderRadius: 3 }}>
                    <Typography>Trạng thái:</Typography>
                    <Typography sx={{ fontWeight: 700, color: 'primary.main' }}>Mở cửa 24/7</Typography>
                  </Box>
                </Stack>
                ...
                <Button 
                  ...
                  target="_blank"
                  sx={{ py: 1.5, borderRadius: 3, fontWeight: 800, bgcolor: 'primary.main' }}
                >
                  Chỉ đường qua Maps
                </Button>
              </Paper>
