...
export default function EHREngine() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: 'background.paper', color: 'text.primary', overflowX: 'hidden' }}>
      <Navbar />
                ...
                <Paper sx={{ 
                  ...
                  <Box sx={{ position: 'relative', zIndex: 1 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 4 }}>
                      ...
                      <Typography variant="overline" sx={{ fontWeight: 800, color: 'text.secondary' }}>{t('ehr_engine_page.sim_source')}</Typography>
                    </Box>
                    <Box sx={{ p: 4, background: 'background.default', borderRadius: '20px', border: '1px solid #e2e8f0', mb: 4 }}>
                      <Typography variant="body2" sx={{ color: 'primary.900', fontFamily: 'serif', lineHeight: 1.8, fontStyle: 'italic' }}>
                        {isEn ? (
                          <>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                      {[
                        ...
                        { label: t('ehr_engine_page.sim_tag_vitals'), value: t('ehr_engine_page.sim_val_bp'), color: 'var(--color-info)' },
                      ].map((tag, idx) => (
                        <motion.div 
                          key={idx}
                        ...
                        >
                          <Box sx={{ p: 2, borderRadius: '12px', background: 'background.paper', border: `1px solid ${tag.color}33`, display: 'flex', flexDirection: 'column' }}>
                            <Typography variant="caption" sx={{ color: tag.color, fontWeight: 800 }}>{tag.label}</Typography>
                            <Typography variant="body2" sx={{ fontWeight: 700, color: 'text.primary' }}>{tag.value}</Typography>
                          </Box>
                        </motion.div>
                      ))}
                    </Box>
              ...
              <Paper sx={{ 
                ...
                <Box sx={{ mt: 'auto', pt: 4, borderTop: '1px solid color-mix(in srgb, var(--color-surface-50) 10%, transparent)' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
                    ...
                    <Typography variant="h4" sx={{ fontWeight: 900, color: 'background.paper' }}>HL7 / FHIR</Typography>
                  </Box>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        {[
                          ...
                          <Box key={idx} sx={{ display: 'flex', gap: 3, alignItems: 'center' }}>
                            <Box sx={{ width: 80 }}>
                              <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 800 }}>{item.date}</Typography>
                            </Box>
                            ...
                            <Box>
                              <Typography variant="body1" sx={{ fontWeight: 700, color: 'primary.900' }}>{item.event}</Typography>
      ...
      <Box sx={{ py: 20, px: 3, background: 'background.paper' }}>
        <Container maxWidth="xl">
          <Box sx={{ textAlign: 'center', mb: 12 }}>
            ...
            <Typography variant="h2" sx={{ fontWeight: 900, mt: 2, color: 'primary.900' }}>
              ...
              <Grid size={{ xs: 12, sm: 6, md: 3 }} key={idx}>
                <Box sx={{ textAlign: 'center', position: 'relative' }}>
                  <Box sx={{ 
                    ...
                    mx: 'auto', mb: 3, color: 'primary.900', border: '2px solid #e2e8f0',
                    transition: 'all 0.3s ease', '&:hover': { borderColor: '#08bba3', background: 'background.paper', color: '#08bba3' }
                  }}>
                    {step.icon}
                  </Box>
                  <Typography variant="h5" sx={{ fontWeight: 800, mb: 1, color: 'primary.900' }}>{step.title}</Typography>
                  <Typography variant="body2" sx={{ color: 'text.secondary', maxWidth: '20ch', mx: 'auto' }}>{step.desc}</Typography>
                </Box>
              </Grid>
            ))}
          </Grid>
