...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public AppointmentResponse updateAppointmentStatus(Long appointmentId, Long doctorId, UpdateAppointmentStatusRequest request) {
        ...
        if (newStatus == AppointmentStatus.IN_PROGRESS) {
            ...
            if (!inProgress.isEmpty() && !inProgress.get(0).getId().equals(appointmentId)) {
                throw new com.caretriage.shared.exception.ConflictException("Bạn đang có một ca khám chưa kết thúc. Vui lòng hoàn thành ca cũ trước.");
            }
        }
    ...
    public AppointmentResponse createAppointmentFromTicket(Long doctorId, CreateAppointmentFromTicketRequest request) {
        User actor = userRepository.findById(doctorId)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy thông tin bác sĩ thực hiện"));
        
        boolean isDoctorOrAdmin = actor.getRoles().stream().anyMatch(r -> r.getName().equals("DOCTOR") || r.getName().equals("ADMIN"));
        if (!isDoctorOrAdmin) {
            throw new org.springframework.security.access.AccessDeniedException("Bạn không có quyền thực hiện hành động này");
        }
        ...
        if (ticket.getDoctorReviewStatus() != TriageTicket.DoctorReviewStatus.DOCTOR_CONFIRMED
                && ticket.getDoctorReviewStatus() != TriageTicket.DoctorReviewStatus.DOCTOR_EDITED) {
            throw new BusinessException("Bác sĩ cần xác nhận hoặc điều chỉnh phiếu phân luồng trước khi tạo lịch hẹn");
        }

        if (appointmentRepository.existsByTriageTicketId(request.getTicketId())) {
            throw new com.caretriage.shared.exception.ConflictException("Phiếu này đã có lịch hẹn trước đó");
        }
        ...
        // Khóa phiên chat session liên kết và cập nhật trạng thái review của ticket
        ticket.setDoctorReviewStatus(TriageTicket.DoctorReviewStatus.APPOINTMENT_CREATED);
        triageTicketRepository.save(ticket);

        if (ticket.getChatSession() != null) {
            com.caretriage.domain.entity.ChatSession session = ticket.getChatSession();
            session.setStatus(com.caretriage.domain.entity.ChatSession.SessionStatus.COMPLETED);
