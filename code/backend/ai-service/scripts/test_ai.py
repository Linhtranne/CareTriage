...
async def run_triage_test(triage_service, name, user_input, expected_goal):
    ...
    print(
        f"\nAI CLINICAL REASONING:\n{reasoning if reasoning else '(No reasoning found)'}",
        flush=True,
    )
    ...
    if result["triage_result"]:
        print(
            f"STRUCTURED DATA: {json.dumps(result['triage_result'], ensure_ascii=False, indent=2)}",
            flush=True,
        )
...
async def run_ner_test(client, name, user_input):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=PromptRegistry.NER_SYSTEM_PROMPT
        ),
    )
