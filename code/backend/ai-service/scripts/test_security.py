...
def test_missing_auth():
    ...
    try:
        response = client.post(
            "/api/triage/analyze", json={"session_id": "test", "message": "hello"}
        )
...
def test_invalid_schema_length():
    response = client.post(
        "/api/triage/analyze",
        json={"session_id": "test", "message": "a" * 5000},
        headers={"X-Internal-Api-Key": settings["internal_api_key"]},
    )
...
def test_negated_red_flag_no_bypass():
    ...
    res1 = RedFlagDetector.detect_red_flags(
        "Tôi không bị đột quỵ và không hề có triệu chứng méo miệng"
    )
    assert res1 is None

    # 2. Negated but with second active emergency symptom: should trigger for second symptom (DYSPNEA)
    res2 = RedFlagDetector.detect_red_flags(
        "Tôi không bị đau ngực nhưng tôi đang khó thở nặng không thở được"
    )
...
def test_web_research_disabled():
    response = client.post(
        "/api/triage/research",
        json={"patient_id": 1, "query": "sốt xuất huyết"},
        headers={"X-Internal-Api-Key": settings["internal_api_key"]},
    )
