...
from app.application.usecases.ehr_extraction_use_case import (  # noqa: E402
    EhrExtractionUseCase,  # noqa: E402
)
from app.domain.schemas import EntityType  # noqa: E402
...
async def test_extract_file_pdf_success(mock_generate, mock_parse_pdf):
    ...
    mock_generate.return_value = {
        "entities": [
            {"entity_type": "SYMPTOM", "entity_value": "fever", "confidence_score": 0.9}
        ]
    }
...
async def test_extract_text_gemini_error_safe_response(mock_generate):
    ...
    response = client.post(
        "/api/ehr/extract-text", json={"text": "Hello clinical note"}
    )
    assert response.status_code == 500
    # Assert public response has safe detail rather than raw Exception message
    assert (
        "Internal AI service error occurred during extraction"
...
async def test_extract_file_parse_error_safe_response(mock_parse_pdf):
    ...
    assert response.status_code == 500
    assert (
        "Internal AI service error occurred during file extraction"
        in response.json()["detail"]
    )
...
def test_parse_entities_malformed_json_recovery():
    ...
    assert res3[0].confidence_score == pytest.approx(0.85)
