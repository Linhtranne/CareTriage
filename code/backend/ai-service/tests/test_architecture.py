    ...
    for module_name in domain_modules:
        ...
        with open(mod.__file__, "r", encoding="utf-8") as f:
            ...
            assert "app.application" not in content, (
                f"{module_name} violates boundary: imports app.application"
            )
            assert "app.infrastructure" not in content, (
                f"{module_name} violates boundary: imports app.infrastructure"
            )
            assert "app.api" not in content, (
                f"{module_name} violates boundary: imports app.api"
            )
